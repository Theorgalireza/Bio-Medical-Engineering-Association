...
export class AnnouncementsController {
  ...
  create(@Body() dto: CreateAnnouncementDto, @CurrentUser() user: User, @Req() req: any) {
    return this.service.create(dto, user, req.user?.id ?? null, req.user?.email ?? null, req.ip);
  }

  @Patch(':id')
  @Roles(Role.ADMIN, Role.OWNER, Role.CONTENT_EDITOR)
  update(@Req() req: any, @Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateAnnouncementDto) {
